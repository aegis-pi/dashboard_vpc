...
import { FACTORIES, FLEET_RECENT, LEVEL_META, buildHistory, fleetCounts, relTime } from "./data.jsx";
import { Icon } from "./icons.jsx";
import { EmptyNote, LevelBadge, PipelineBadge, SectionHeader, StalenessBadge } from "./shared.jsx";
  ...
  return (
    <>
      ...
      <div className="card">
        <SectionHeader
          title="최근 상태 변화"
          hint={`HISTORY#RISK · risk_level 변화 ${FLEET_RECENT.length}건`} />
        
        <div>
          {FLEET_RECENT.length === 0 ?
          <EmptyNote text="최근 24시간 내 변화가 없습니다." /> :
          FLEET_RECENT.map((e, i) =>
...
function FleetPulse({ demoMode = "off" }) {
  ...
  const factories = applyPulseDemo(FACTORIES, demoMode);

  // The three bands: 0–50 위험, 50–85 주의, 85–100 안전
  const bands = [
    ...
    <div className="card" style={{ marginBottom: 18 }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <StatDivider />
          <PulseStat label="위험" value={counts.danger} tone="crit" dotColor="var(--crit)" />
          <StatDivider />
          <PulseStat label="주의" value={counts.warning} tone="warn" dotColor="var(--warn)" />
          <StatDivider />
          <PulseStat label="안전" value={counts.safe} tone="safe" dotColor="var(--safe)" />
          <StatDivider />
          <PulseStat label="데이터 지연" value={counts.stale} tone="warn" dotColor="var(--warn)" />
        </div>
      </div>
      ...
      <div style={{ padding: "4px 22px 18px" }}>
        <div style={{
          position: "relative", padding: "6px 6px 0",
          ...
          <div style={{
            ...
            <div style={{ position: "absolute", inset: 0, display: "flex" }}>
              <BandLabel from={0} to={50} color="var(--crit)" label="위험" />
              <BandLabel from={50} to={85} color="var(--warn)" label="주의" />
...
function computeLabelPositions(dots, minGap = 11) {
  ...
  for (let i = 1; i < dots.length; i++) {
    if (dots[i] - dots[i - 1] < minGap) cur.push(i);else
    {clusters.push(cur);cur = [i];}
  }
  clusters.push(cur);
...
function BandLabel({ from, to, color, label }) {
  return (
    <div style={{
      position: "absolute", left: `${from}%`, width: `${to - from}%`,
      top: 0, bottom: 0,
      ...
      <span className="mono" style={{
        ...
        color, fontWeight: 700, opacity: 0.95
      }}>{label}</span>
    </div>);

}

// Vertical divider for the stat group on the pulse header.
...
function PulseStat({ label, value, tone, dotColor }) {
  ...
  const numberColor = active ? palette[tone] ?? "var(--ink)" : "var(--ink-3)";
  return (
    <div style={{
      display: "inline-flex", alignItems: "center",
      gap: 5,
      ...
      <span className="tnum" style={{
        fontSize: 12,
        fontWeight: 500, color: numberColor,
        letterSpacing: "-0.005em", lineHeight: 1,
        fontVariantNumeric: "tabular-nums",
        fontFeatureSettings: "normal"
      }}>{value}</span>
      <span style={{
        fontFamily: '"Pretendard", "Apple SD Gothic Neo", "Noto Sans KR", "Geist", sans-serif',
        fontSize: 11,
        color: "var(--ink-2)",
        fontWeight: 400,
        letterSpacing: 0,
      ...
      <div style={{ padding: "16px 18px 8px", display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        <div style={{
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
            ...
            <span className="micro" style={{ marginTop: 6, whiteSpace: "nowrap" }}>
              /100 · 안전 점수
            </span>
          </div>
...
function RecentRow({ e, onOpen }) {
  const fromMeta = LEVEL_META[e.from];
  const toMeta = LEVEL_META[e.to];
