...
import { FACTORIES, REPORTS, REPORT_DATES, TODAY, canAccessFactory, relTime } from "./data.jsx";
...
function ReportsPage({ role }) {
  const [factoryId, setFactoryId] = React.useState(FACTORIES[0].factory_id);
  ...
  React.useEffect(() => {
    if (!canAccessFactory(role, factoryId)) {
      const fb = FACTORIES.find(f => canAccessFactory(role, f.factory_id));
      if (fb) setFactoryId(fb.factory_id);
    }
  }, [role, factoryId]);

  const report = REPORTS[factoryId]?.[date];
    ...
    return (
      <ReportState icon="refresh" tone="warn" title="보고서 생성 중"
        ...
        <span className="mono micro">
          queued_at <span style={{ color: "var(--ink-3)" }}>{relTime(report.queued_at)}</span>
        </span>
      </ReportState>
    );
  }
  if (report.status === "failed") {
    return (
      <ReportState icon="alert" tone="crit" title="보고서 생성 실패"
        ...
        <span className="mono micro">
          attempted_at <span style={{ color: "var(--ink-3)" }}>{relTime(report.generated_at)}</span>
        </span>
        {role === "admin" && (
  ...
  return (
    <div className="card">
      <SectionHeader
        ...
        hint={`generated ${relTime(report.generated_at)}`}
      />
