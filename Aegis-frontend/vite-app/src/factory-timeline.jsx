...
import { EVENT_KIND_META, TIMELINE, clockHHMM, relTime } from "./data.jsx";
import { EmptyNote, SectionHeader } from "./shared.jsx";
...
function FactoryTimeline({ f }) {
  const events = (TIMELINE[f.factory_id] || [])
    .slice()
    .sort((a, b) => b.ts - a.ts);
  ...
  return (
    <div className="card">
      ...
      <div style={{
        ...
        {Object.entries(EVENT_KIND_META).map(([k, m]) => (
...
function TimelineRow({ e, last }) {
  const meta = EVENT_KIND_META[e.kind] || { label: e.kind, icon: "events" };
  const tone =
