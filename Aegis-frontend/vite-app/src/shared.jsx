...
import { LEVEL_META, PIPELINE_META, ageSeconds, relTime } from "./data.jsx";
...
function StalenessBadge({ factory }) {
  const fAge = ageSeconds(factory.last_factory_state_at);
  const iAge = ageSeconds(factory.last_infra_state_at);
    ...
    <div style={{
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: "flex", justifyContent: "space-between", gap: 8, alignItems: "baseline" }}>
          <span style={{ fontSize: 12.5, color: "var(--ink)", fontWeight: 500, whiteSpace: "nowrap" }}>{label}</span>
          <span className="micro" style={{ whiteSpace: "nowrap" }}>{text}</span>
        </div>
        <div className="micro mono" style={{ marginTop: 2, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
          {lastSeen
            ? <>last_seen <span style={{ color: "var(--ink-2)" }}>{relTime(lastSeen)}</span></>
...
export { DeviceChip, EmptyNote, KPITile, LevelBadge, PipelineBadge, SectionHeader, StalenessBadge, showVal };
