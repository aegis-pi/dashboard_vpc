...
// bands: optional [{from, to, color}] — tinted horizontal bands.
// thresholdLines: optional [{y, color}] — dashed horizontal threshold lines.
function MultiLine({ series, height = 200, yUnit = "", yTicks = 4,
                     ...
                     bands = null, thresholdLines = null }) {
  const ref = React.useRef(null);
  const [w, setW] = React.useState(0);
  React.useEffect(() => {
    if (!ref.current) return;
  ...
  return (
    <div ref={ref} style={{ width: "100%", height }}>
      <svg width="100%" height={H} viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none">
        {/* tinted threshold bands (drawn under gridlines) */}
        {bands && bands.map((b, bi) => {
          const yTop = yOf(Math.min(b.to, yMax));
          const yBot = yOf(Math.max(b.from, yMin));
        ...
        {/* threshold lines — clean solid 1px, slight tint */}
        {thresholdLines && thresholdLines.map((t, ti) => (
          <line key={`th${ti}`}
                x1={pad.l} x2={W - pad.r}
                y1={yOf(t.y)} y2={yOf(t.y)}
                stroke={t.color} strokeWidth="1"
                strokeOpacity="0.4" />
        ))}
        {/* y labels */}
        {yTickVals.map((v, i) => (
...
export { Heatmap, LineChart, MultiLine, RiskGauge, ScoreBar, Sparkline, StackedRisk, StatusDonut };
