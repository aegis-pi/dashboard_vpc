...
```json
{
  ...
  "fast_stale": false,
  "slow_stale": false,
  "stale_threshold_seconds": { "fast": 180, "slow": 900 },
...
staleness 기준(doc 29 / 코드): `fast` age > 180초, `slow` age > 900초 → 해당 쪽 section의 `status`를 `unknown`으로 강등하고, `overall_status`가 `normal`이면 `warning`으로 올린다. 임계값은 응답의 `stale_threshold_seconds`로 함께 내려준다. `*_updated_at`이 없거나 파싱 불가하면 stale로 간주한다(`*_age_seconds=null`).

구현 위치: `services/cloud_infra.py`의 `_status_with_staleness()`. item이 없으면 `get_latest()`가 `{ "available": false }`를 반환한다.
