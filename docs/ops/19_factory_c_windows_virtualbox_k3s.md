...
Windows 호스트에서 VirtualBox 기반 Linux VM **2 대**를 만들고 `factory-c` 테스트베드형 Spoke K3s 를 **server + agent 2-노드 클러스터** 로 상시 무인 운영 가능하게 구성한다.

이 문서는 운영자가 다른 문서를 함께 열지 않고도 처음부터 끝까지 따라갈 수 있도록 작성되었다. 목표는 두 VM 으로 이뤄진 `factory-c` 클러스터가 IoT Core 로 `aegis/factory-c/*` topic 데이터를 송신하고, IoT Rule (ADR 0018) 을 통해 S3 `raw/factory-c/...` 에 적재되며, Hub/ArgoCD 가 Tailscale 을 통해 `factory-c` K3s API (server 노드) 를 인식하는 기준선을 만드는 것이다.

2-VM 구성으로 가는 이유와 변경 근거는 `docs/changes/0019-factory-c-server-agent-cluster.md` 참조. 워크스트림 A↔B 합류 지점인 IoT Rule 확장은 `docs/changes/0018-iot-rule-extend-to-factory-c.md` 참조.
...
| **Computer 3** | 상시 운영 PC (신규, 백지 Windows) | VirtualBox 호스팅. 안에 `factory-c` Ubuntu VM **2 대** (`factory-c-server`, `factory-c-agent`). 절대 끄지 않는다 |
...
| **Computer 3 VM (server)** | factory-c-server. Ubuntu Server, K3s server (control plane), Tailscale, IoT 인증서 cluster Secret 위치 |
| **Computer 3 VM (agent)** | factory-c-agent. Ubuntu Server, K3s agent (worker), Tailscale, dummy publisher systemd, Edge Agent dummy mode 배치 노드 |
...
| VirtualBox VM **2 개** 자동 시작 (Task Scheduler) | Step 15 에서 설정 (server / agent 각각) |
...
| 항목 | server | agent |
| --- | --- | --- |
| VM 이름 / hostname | `factory-c-server` | `factory-c-agent` |
| vCPU | 2 | 2 |
| RAM | 2 GiB | 4 GiB |
| Disk | 20 GiB | 40 GiB |
...
| Server VM / 노드명 | `factory-c-server` (control-plane, 워크로드 미배치) |
| Agent VM / 노드명 | `factory-c-agent` (worker, dummy publisher + Edge Agent 배치) |
| Spoke type | `testbed` |
| Environment type | `vm-windows` |
| Input module type | `dummy` (agent 노드 label) |
...
| Tailscale hostnames | `factory-c-server`, `factory-c-agent` |
| ArgoCD cluster context | `factory-c` (Step 12 에서 rename, server API 기준) |
...
register-thing.sh  ──── AWS IoT Thing
   └─ secret/iot/factory-c/  ─── scp ─→ server: /etc/aegis/iot/factory-c/  (Step 8 Secret 등록용)
                                        agent : /etc/aegis/iot/factory-c/  (Step 9 smoke / Step 11 publisher)
...
kubeconfig 수신 ←─── scp ─────────────── server: ~/.kube/config 사본
   └─ server 주소를 server Tailscale IP 로 rewrite
...
aws s3 ls raw/factory-c/                AWS S3 (IoT Rule 통해)
   └─ factory_state, infra_state 누적 확인 (nodes 배열 길이 == 2)
                                       ...
                                       절전 OFF · 자동 로그인
                                       Task Scheduler:
                                         Start factory-c-server VM  (boot 시 즉시)
                                         Start factory-c-agent VM   (boot + 60s)
```

데이터 경로:

```text
factory-c-agent VM (systemd dummy publisher 또는 후속 Edge Agent dummy mode)
  -> AWS IoT Core
...
```text
Hub EKS / ArgoCD  -> Tailscale  -> factory-c-server K3s API (6443)
...
- `infra_state.payload`: `heartbeat`, `cluster`, `nodes` (**길이 2**: server + agent), `workloads`, `devices`

## Step 1. VirtualBox 네트워크 결정

실행 위치: **Computer 3 Windows 호스트**

권장은 **NAT + Tailscale** (두 VM 모두). 이유: Computer 1 ↔ Computer 3 VM 사이 SSH 경로를 Tailscale IP 로 안정화할 수 있고, server ↔ agent 간 K3s API (6443) 트래픽도 Tailscale 안에서 처리된다.

| 방식 | 사용 시점 | 장점 | 주의 |
| --- | --- | --- | --- |
| **NAT + Tailscale (권장)** | 무인 상시 운영 | Tailscale IP 안정, server↔agent 간 6443 도 Tailscale 안 | Tailscale 참여 전까지 SSH/Join 불편 |
...
VM 두 개를 만든다. 사이즈를 서로 다르게 둔다 (server 는 control plane 만 돌아서 작게, agent 가 워크로드 받음).

### 2A. VM `factory-c-server`
...
```text
hostname: factory-c-server
ssh:      enabled
user:     운영자가 정한 일반 사용자 (예: aegis)
```

### 2B. VM `factory-c-agent`
...
hostname 이 다르면 고정 (`factory-c-server` 또는 `factory-c-agent`):

```bash
# server VM
sudo hostnamectl set-hostname factory-c-server

# agent VM
sudo hostnamectl set-hostname factory-c-agent
```
...
```text
Static hostname: factory-c-server  (또는 factory-c-agent)
SSH service: active
...
tailscale ip -4    # 이 값을 메모. 이후 TS_IP_SERVER 로 표기 (Step 5, 8, 12 에서 사용)
...
sudo tailscale up \
  ...
  --hostname=factory-c-agent
tailscale status --self
tailscale ip -4    # 이 값을 메모. 이후 TS_IP_AGENT 로 표기 (Step 9, 11 에서 사용)
unset TAILSCALE_AUTH_KEY
history -c
```
...
```text
hostname:     factory-c-server  (또는 factory-c-agent)
tags:         tag:aegis-spoke-testbed, tag:factory-c
tailscale IP: 100.x.y.z
```

> 이후 명령에서 두 IP 가 자주 등장하므로 명확히 구분해 메모해 둔다.
> - `TS_IP_SERVER` = factory-c-server 의 Tailscale IP
> - `TS_IP_AGENT`  = factory-c-agent  의 Tailscale IP

## Step 5. K3s 설치 (server → agent)

server 가 먼저 떠 있어야 agent 가 join 할 수 있다. 두 단계로 나눠 진행한다.

### 5-1. server 설치 (실행 위치: server VM)

```bash
TS_IP_SERVER="$(tailscale ip -4)"
echo "TS_IP_SERVER=${TS_IP_SERVER}"

# server 설치 + Tailscale IP 를 TLS SAN 에 포함
curl -sfL https://get.k3s.io | \
  INSTALL_K3S_EXEC="server --node-name factory-c-server --tls-san ${TS_IP_SERVER}" \
  sh -
...
```text
factory-c-server  Ready  control-plane,master
...
```bash
kubectl get node factory-c-server -o jsonpath='{.status.nodeInfo.kubeletVersion}{"\n"}'
# 예: v1.34.6+k3s1
```

### 5-2. agent 설치 (실행 위치: agent VM)

```bash
TS_IP_AGENT="$(tailscale ip -4)"
echo "TS_IP_AGENT=${TS_IP_AGENT}"

# Step 5-1 에서 얻은 server Tailscale IP 와 token 을 변수로 받는다 (silent input)
read -r -p "TS_IP_SERVER (server Tailscale IP): " TS_IP_SERVER
read -r -s -p "K3S_TOKEN: " K3S_TOKEN; echo
export K3S_TOKEN

curl -sfL https://get.k3s.io | \
  K3S_URL="https://${TS_IP_SERVER}:6443" \
  K3S_TOKEN="${K3S_TOKEN}" \
  INSTALL_K3S_EXEC="agent --node-name factory-c-agent --node-ip ${TS_IP_AGENT}" \
  sh -
...
### 5-3. cluster 전체 확인 (실행 위치: server VM)
...
factory-c-server    Ready    control-plane,master   ...   v1.34.x+k3s1
factory-c-agent     Ready    <none>                 ...   v1.34.x+k3s1
```

`factory-c-agent` 가 Ready 가 아니면 → 문제 해결 § "agent 가 server 에 join 못함" 참고.

## Step 6. Factory C node label 적용

실행 위치: **server VM** (server 의 kubectl 로 두 노드 모두 label)

```bash
# 두 노드 공통 label
for n in factory-c-server factory-c-agent; do
...
# 입력 모듈 label 은 워크로드 받는 agent 노드에만
kubectl label node factory-c-agent aegis.input-module-type=dummy --overwrite
...
```text
factory-c-server : aegis.factory-id=factory-c, aegis.environment-type=vm-windows, aegis.spoke-type=testbed
                   (input-module-type 라벨 미부착)
                   taint: node-role.kubernetes.io/control-plane:NoSchedule  (K3s 기본)
factory-c-agent  : 위 3개 + aegis.input-module-type=dummy
                   (no taint)
```

이렇게 두면 후속 Edge Agent dummy mode (Step 14) 가 `nodeSelector: aegis.input-module-type=dummy` 또는 단순히 control plane taint 회피로 자연스럽게 agent 노드에 배치된다.
...
## Step 8. IoT 인증서를 K3s Secret 으로 주입 (cluster scope, server VM)

실행 위치: **Computer 1 (WSL) → Computer 3 server VM** (스크립트가 SSH 로 server 에 들어가 `kubectl` 실행)

Secret 은 cluster scope (`ai-apps` namespace) 라 server 에서 한 번 등록하면 agent 에 스케줄된 pod 도 그대로 마운트한다.

### 사전 조건: server VM SSH 키 인증 (1회만)

```bash
# Computer 1 (WSL) 에서
TS_IP_SERVER=<Step 4A 에서 메모한 server Tailscale IP>
ssh-copy-id <vm-ssh-user>@${TS_IP_SERVER}
ssh <vm-ssh-user>@${TS_IP_SERVER} 'whoami && which kubectl'
```

`which kubectl` 가 빈 결과면 비-interactive SSH PATH 에 `/usr/local/bin` 이 빠진 것이다. server VM 의 `~/.bashrc` 또는 `~/.profile` 맨 위에 다음 줄을 추가:
...
## Step 9. agent VM 에서 MQTT smoke publish

실행 위치: **Computer 1 → Computer 3 agent VM** (인증서 사본 전달 후 agent 에서 publish)

dummy publisher 는 Step 11 에서 agent VM 에 systemd 로 띄울 예정이므로, 인증서 사본은 처음부터 agent VM 에 둔다 (server VM 에는 Step 8 자동 스크립트가 이미 cluster Secret 용도로 임시 복사했고 자동 삭제됨 — agent VM 에는 영구적으로 둔다).

### 인증서 사본 전달 (Computer 1 → agent VM)

```bash
TS_IP_AGENT=<Step 4B 에서 메모한 agent Tailscale IP>

ssh <vm-ssh-user>@${TS_IP_AGENT} 'sudo mkdir -p /etc/aegis/iot/factory-c'

scp secret/iot/factory-c/certificate.pem.crt \
    secret/iot/factory-c/private.pem.key \
    secret/iot/factory-c/AmazonRootCA1.pem \
    secret/iot/factory-c/endpoint.txt \
    <vm-ssh-user>@${TS_IP_AGENT}:/tmp/

ssh <vm-ssh-user>@${TS_IP_AGENT} '
...
### factory_state publish (agent VM)

```bash
# agent VM 에 SSH 로 들어와서
sudo -i
...
jq -n \
  ...
  '{
    ...
    message_id: ("factory-c:factory_state:factory-c-agent:" + $ts),
    factory_id: "factory-c",
    node_id: "factory-c-agent",
...
### infra_state publish (agent VM, 두 노드 포함)

```bash
# 여전히 sudo -i 안. server / agent 의 K3s 버전을 미리 채집
# server 에 ssh 해 가져오거나 (agent VM 에 kubectl 없음), 또는 단순히 5-1 에서 기록해 둔 값을 변수에 직접 넣는다
...
input_module_type == "dummy"
nodes_len == 2
nodes == ["factory-c-server", "factory-c-agent"]
```

빈 결과면 → 문제 해결 § "S3 `raw/factory-c` 가 비어 있음 (Step 10 이후)" 참고.

## Step 11. factory-c 가데이터 자동 발행 루프 (agent VM)

실행 위치: **Computer 3 agent VM**
...
publisher 는 cluster 의 worker 노드인 **agent VM 에 1개만** 띄운다 (server VM 에 두지 않는다). cluster 단위 1회 publish 면 충분하다.
...
agent VM 에는 kubectl 이 기본으로 없다 (k3s agent install 은 server 처럼 kubectl 심볼릭 링크를 만들지 않음). 매번 cluster 에 질의하지 않도록 K3s 버전을 install 시점에 한 번만 채집해 파일로 저장한다.
...
`/usr/local/bin/aegis-dummy-publisher.sh` 는 `noisy-vm` 프로파일대로 값을 흩어 두 토픽으로 publish 하는 무한 루프다. `infra_state.payload.nodes` 배열에 server / agent 두 노드를 함께 보낸다.
...
publish_infra_state() {
  ...
  # server / agent 노드 메트릭 (synthetic — testbed 용)
...
[Unit]
Description=Aegis factory-c dummy publisher (testbed, agent VM)
...
.payload.nodes node_id 목록 = ["factory-c-server", "factory-c-agent"]
```

## Step 12. Tailscale IP 기반 kubeconfig 생성 + Context rename

실행 위치: **Computer 3 server VM → Computer 1 (WSL)**

cluster 단위 kubeconfig 는 server 의 kubeconfig 를 외부에서 쓸 수 있게 만든다.

### Computer 3 server VM 에서: kubeconfig 임시 사본
...
scp <vm-ssh-user>@${TS_IP_SERVER}:/tmp/factory-c.kubeconfig \
    ~/.aegis/secrets/kubeconfig/factory-c.kubeconfig

# server 주소를 127.0.0.1 -> server Tailscale IP 로 변경
sed -i "s|server: https://127\.0\.0\.1:6443|server: https://${TS_IP_SERVER}:6443|" \
    ~/.aegis/secrets/kubeconfig/factory-c.kubeconfig
...
TLS 오류가 나면 Step 5-1 에서 `--tls-san ${TS_IP_SERVER}` 가 빠진 것이다. 초기 구축 단계라면 K3s server 재설치가 가장 단순하다 — `문제 해결` 섹션 참고.
...
```text
SERVER                          NAME        STATUS
https://<TS_IP_SERVER>:6443     factory-c   Successful
```
...
dummy_data:
  profile: noisy-vm
nodeSelector:
  aegis.input-module-type: dummy   # agent 노드에만 배치
```

배포 후 확인 (Computer 1 의 factory-c kubeconfig 또는 server VM 에서):
...
```text
edge-agent Pod: Running, NODE=factory-c-agent  (agent 노드 배치)
mode: dummy
factory_id: factory-c
...
이 단계는 Computer 3 가 정전/재부팅/Windows Update 후 사람 개입 없이 server VM → agent VM 까지 자동으로 올라오게 만든다.
...
server / agent 각각 작업을 만든다. agent 작업은 server 가 먼저 떠 있도록 60초 지연을 둔다.

#### 15-3A. `Start factory-c-server VM`

1. 시작 메뉴 → "작업 스케줄러" → **기본 작업 만들기**
   - 이름: `Start factory-c-server VM`
   - 설명: `VirtualBox factory-c-server headless autostart`
2. **트리거**: "컴퓨터 시작 시"
3. **동작**: "프로그램 시작"
   - 프로그램/스크립트: `"C:\Program Files\Oracle\VirtualBox\VBoxManage.exe"`
   - 인수 추가: `startvm "factory-c-server" --type headless`
4. 마지막 화면 "[속성] 대화 상자 열기" 체크 후 **마침**
5. 속성:
   - "사용자가 로그온했는지 여부에 관계없이 실행" + "가장 높은 수준의 권한으로 실행" 둘 다 체크
   - 설정 탭 → "작업이 실패하는 경우 다시 시작 간격: 1분, 횟수: 3"

#### 15-3B. `Start factory-c-agent VM` (60초 지연)

1. 같은 절차로 작업 추가
   - 이름: `Start factory-c-agent VM`
   - 설명: `VirtualBox factory-c-agent headless autostart (after server)`
2. **트리거**: "컴퓨터 시작 시"
3. **트리거 → 고급 설정**: "지연 시간: **1분**" 체크 (60초 후 시작)
4. **동작**: "프로그램 시작"
   - 프로그램/스크립트: `"C:\Program Files\Oracle\VirtualBox\VBoxManage.exe"`
   - 인수 추가: `startvm "factory-c-agent" --type headless`
5. 속성:
...
```powershell
# 두 VM 종료
& "C:\Program Files\Oracle\VirtualBox\VBoxManage.exe" controlvm "factory-c-agent"  poweroff
& "C:\Program Files\Oracle\VirtualBox\VBoxManage.exe" controlvm "factory-c-server" poweroff

# 작업 스케줄러로 server 먼저 즉시 실행
schtasks /Run /TN "Start factory-c-server VM"
Start-Sleep -Seconds 60
schtasks /Run /TN "Start factory-c-agent VM"

# VM 상태 확인
& "C:\Program Files\Oracle\VirtualBox\VBoxManage.exe" list runningvms
```

`factory-c-server`, `factory-c-agent` 둘 다 list 에 보이면 성공.
...
1. Windows 자동 로그인 완료
2. Task Scheduler `Start factory-c-server VM` 실행 → server VM 부팅 → K3s server + Tailscale active
3. (60초 후) Task Scheduler `Start factory-c-agent VM` 실행 → agent VM 부팅 → K3s agent join + Tailscale + `aegis-dummy-publisher.service` active

### Computer 1 (WSL) 에서 외부 검증 (약 3~4분 후)

```bash
TS_IP_SERVER=<server Tailscale IP>
TS_IP_AGENT=<agent  Tailscale IP>

# Tailscale 도달 확인
ping -c 3 ${TS_IP_SERVER}
ping -c 3 ${TS_IP_AGENT}
...
k3s: active
tailscaled: active
factory-c-server Ready
factory-c-agent  Ready
label: server 에 aegis.factory-id=factory-c, environment-type=vm-windows, spoke-type=testbed
       agent  에 위 3개 + aegis.input-module-type=dummy
```
...
- [ ] `argocd cluster list` 에 `factory-c` Successful (server API)

### Computer 3 Windows 호스트

- [ ] VirtualBox 설치
- [ ] VM `factory-c-server` (2 vCPU / 2 GiB / 20 GiB) 생성
- [ ] VM `factory-c-agent`  (2 vCPU / 4 GiB / 40 GiB) 생성
- [ ] 절전/슬립 영구 OFF
- [ ] Windows 자동 로그인 enabled
- [ ] Task Scheduler `Start factory-c-server VM` 등록 + 검증 완료
- [ ] Task Scheduler `Start factory-c-agent VM` 등록 (60초 지연) + 검증 완료
- [ ] Windows 재부팅 후 사람 개입 없이 두 VM 부팅까지 자동 완료
- [ ] (권장) Tailscale Windows 클라이언트 연결

### Computer 3 server VM (factory-c-server)
...
### Computer 3 agent VM (factory-c-agent)
...
- [ ] `infra_state.payload.nodes` 길이 == 2, node_id = `factory-c-server`, `factory-c-agent`
...
- [ ] ADR `docs/changes/0019-factory-c-server-agent-cluster.md` 반영 완료
...
- SSH 키 미등록 → `ssh-copy-id <user>@${TS_IP_SERVER}` 1회 실행 (대상은 server VM)
- VM 의 sshd 미동작 → server VM 에서 `sudo systemctl status ssh` 확인
...
원인: 비-interactive SSH 의 PATH 에 `/usr/local/bin` 미포함. **대상은 server VM**.

해결: server VM 의 `~/.bashrc` 또는 `~/.profile` 맨 위에 `export PATH="$PATH:/usr/local/bin"` 추가.

### agent 가 server 에 join 못함 (Step 5-2 / 5-3)

증상: server 의 `kubectl get nodes` 에 `factory-c-agent` 가 안 보임 또는 NotReady.

원인 후보:

- agent VM 의 `k3s-agent.service` 미동작 → `sudo systemctl status k3s-agent --no-pager` / `journalctl -u k3s-agent -n 100`
- agent → server Tailscale 도달 실패 → agent VM 에서 `curl -k https://${TS_IP_SERVER}:6443/healthz` 결과 확인 (정상이면 `ok`)
- `K3S_TOKEN` 오타 → `sudo cat /var/lib/rancher/k3s/agent/server-token` 으로 agent 가 사용 중인 token 확인 (있는 경우)
- `--node-ip` 누락 → agent 가 NAT 내부 IP 로 광고되어 클러스터 통신 깨짐. install 명령에 `--node-ip ${TS_IP_AGENT}` 가 포함되었는지 확인
- agent 재설치 절차:
...
원인: Step 5-1 K3s server 설치 시 `--tls-san ${TS_IP_SERVER}` 누락.
...
TS_IP_SERVER="$(tailscale ip -4)"
curl -sfL https://get.k3s.io | INSTALL_K3S_EXEC="server --node-name factory-c-server --tls-san ${TS_IP_SERVER}" sh -
...
# server 재설치 후에는 agent 의 token 도 갱신되므로 agent 도 재설치
# (agent VM 에서)  sudo /usr/local/bin/k3s-agent-uninstall.sh  → Step 5-2 재실행
...
| `mosquitto_pub` network 오류 | agent VM 인터넷, DNS, 방화벽 확인 |
| IoT Rule 자체가 없음 | Step 10-2 의 `create-topic-rule` 누락. ADR 0018 절차 재실행 |

### `aegis-dummy-publisher.service` 가 자주 재시작 (agent VM)
...
원인: Step 14 의 dummy publisher 중단 누락 (agent VM).
...
1. 작업 스케줄러 `Start factory-c-server VM` 및 `Start factory-c-agent VM` 둘 다 "준비" 상태인지 (마지막 실행 결과 코드 0)
...
5. agent 작업의 트리거 지연 1분 설정 여부
6. 수동 실행: PowerShell 관리자로 server → 60초 → agent 순서로 `schtasks /Run /TN ...` 후 `VBoxManage list runningvms`
...
```bash
systemctl status k3s --no-pager           # server VM
systemctl status k3s-agent --no-pager     # agent VM
systemctl status tailscaled --no-pager
systemctl status aegis-dummy-publisher --no-pager   # agent VM
...
- Tailscale 실패는 Auth Key 만료 (Reusable 인 경우 이미 등록된 상태이므로 재인증 불필요). 재설치한 경우만 `sudo tailscale up --authkey=... --hostname=factory-c-<server|agent>` 로 재참여
