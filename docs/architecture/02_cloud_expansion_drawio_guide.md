...
| 파일 | 위상 | 비고 |
| --- | --- | --- |
| `drawio/01_re4.drawio` / `images/01_re4.jpg` | historical reference (단일 VPC, ADR 0005 이전) | EKS Hub 안에 Risk Normalizer/Risk Score Engine 컨테이너 표기. 최신 결정과 어긋나므로 사용하지 않음 |
| `drawio/02_re5_two_vpc_target.drawio` | historical reference (pre-Phase 1, 2026-05-15) | 2 VPC 분리, Lambda Dashboard API (VPC 밖), 1번 VPC 비어있음, NAT GW 제거. Phase 1 통합 결정(ADR 0012~0016) 이전 |
| `drawio/03_re6_workstream_b_enhanced.drawio` | **현재 source of truth** (2026-05-19, ADR 0012~0017 반영) | Phase 1 통합 결정 반영. 1번 VPC에 ALB + ECS Fargate Backend + RDS PostgreSQL + Redis + NAT GW × 1 배치. WebSocket(DDB Streams + Lambda notifier + Redis Pub/Sub) + Bedrock 일간 보고서 포함 |
| `drawio/04_re7_data_dashboard_vpc_image_overview.drawio` / `images/04_re7_data_dashboard_vpc_image_overview.jpg` | presentation overview (2026-05-18) | 03_re6 기준을 AWS/service 이미지 아이콘으로 재구성한 발표·보고용 단일 이미지. 선 의미표는 `04_re7_line_legend*.drawio`와 JPG를 함께 사용 |

신규 다이어그램(`03_re6`)은 단일 페이지 Overview 형태다. 후속에 본 가이드의 5개 분할(Overview / Data Plane / Control Plane / CI/CD / Dashboard Access) 권장에 맞춰 페이지를 추가한다. Phase 1 한정 추가 권장 페이지로 **Backend Container Tier**(ECS Backend·RDS PostgreSQL·Redis·notifier 상세)를 별도 페이지로 두면 인터뷰·발표 시 깊이 있는 설명이 가능하다.
