...
- 상태: 부분 완료 (관제 화면 구현/배포 완료, 챗봇 backend/web 로컬 구현 + Bedrock IAM 적용 완료, 운영 rollout·실시간 시나리오 검증 후속)
- 진행 요약: Dashboard VPC 기반 관제 화면을 운영 배포까지 완료했다. Fleet·Factory·Cloud Infra·S3 `reports/daily/` 기반 보고서 조회/Word `.docx` 내보내기·Cognito/RDS RBAC와 `/admin/users`를 제공한다. 추가로 ADR 0033 기준 `/chat/query` backend를 로컬 구현해 RBAC 이후 DDB evidence를 만들고, Bedrock 2-tier(fast Haiku 4.5 / precise Sonnet 4.6) 또는 rule fallback으로 답변한다. dashboard-web에는 Workspace `AI 채팅` 독립 페이지를 추가했다. ECS task role Bedrock IAM은 운영 적용 완료했으며, backend/web 운영 rollout은 후속이다.
- 변경/확인: `apps/dashboard-backend/`(routers: factories·reports·ws·cloud_infra·admin_users·auth_me·chat / services: ddb·redis·s3·cloud_infra·metadata·rbac_seed·cognito_admin·chat·bedrock / deps: auth·rbac / db RBAC 모델; history `since` delta query와 window별 기본 limit; history/GRAPH#5M numeric string 정규화; `cloud-infra` 보고서 system-view 권한 분리; `/chat/query` intent/time parser·Evidence·Bedrock explain·rule fallback), `apps/dashboard-web/`(Fleet·Factory·Reports·CloudInfra·AdminUsers·Login·Callback; history delta merge/dedupe, Factory header 10m trend, WebSocket LATEST chart append; numeric string 정규화; Reports Markdown parser 공용화와 실제 `.docx` export; system-view 사용자용 Cloud Infra report selector), `apps/cloud-infra-collector/`, `infra/data-dashboard/`(ECS Auto Scaling·task role Cognito/Bedrock 권한·metadata/Bedrock env), 관련 ADR 0025/0026/0028/0029/0030/0031/0033, `docs/ops/15_aws_cost_baseline.md`, `docs/ops/22_data_dashboard_vpc_runbook.md`.
- 검증: backend `pytest -q` 146 passed, dashboard-web `npm run lint` 통과, `npm test -- --run` 80 passed, `npm run build` 통과(Vite chunk warning only), `terraform -chdir=infra/data-dashboard fmt -check` 통과, `terraform -chdir=infra/data-dashboard validate` 통과, `git diff --check` 통과. Bedrock IAM targeted apply 완료, `bedrock:InvokeModel`/`bedrock:GetInferenceProfile` simulation allowed. Terraform plan은 `backend_container_image=sha-71bbe1d` 명시 시 task definition env 추가만 남음(서비스는 `task_definition` ignore로 미롤아웃). 운영 API `/healthz` ok, `/readyz` dynamodb/redis/rds_metadata ok, ECS desired/running 2 revision 40 유지.
- 후속: `/chat/query` backend image push/rollout과 dashboard-web 배포/CloudFront invalidation, Cloud Infra UI 보정 배포/수기 캡처, Cognito super_admin bootstrap 후 실제 사용자 생성/권한 수정/삭제 수기 확인, factory-a Edge Agent 재활성화 후 IoT → DDB → Redis → WebSocket 실시간 시나리오(Issue 8) 검증, LLM 일간 보고서 생성기(ADR 0016), 알림룰 관리 화면.
