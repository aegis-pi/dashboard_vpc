...
- 진행 요약: Dashboard VPC 기반 관제 화면을 운영 배포까지 완료했다. Fleet(공장별 위험도 카드)·Factory(센서 현황·이상 항목·WebSocket 실시간 갱신·`10m/1h/custom` 타임라인 + `top_causes` 원인)·Cloud Infra 상태 화면·S3 `reports/daily/` 기반 일간 보고서 조회 화면을 제공하고, Cognito + RDS 기반 공장별 RBAC와 `/admin/users` 관리자 사용자 관리 화면을 추가했다. Risk 계산/Twin 출력 구조는 `apps/data-processor`로 구현되어 cloud-side로 표시된다. LLM 일간 보고서 생성기(Bedrock)와 factory-a 실시간 Edge Agent 기반 시나리오 검증은 별도 후속이다.
- 변경/확인: `apps/dashboard-backend/`(routers: factories·reports·ws·cloud_infra·admin_users·auth_me / services: ddb·redis·s3·cloud_infra·metadata·rbac_seed·cognito_admin / deps: auth·rbac / db RBAC 모델; history `since` delta query와 window별 기본 limit), `apps/dashboard-web/`(Fleet·Factory·Reports·CloudInfra·AdminUsers·Login·Callback; history delta merge/dedupe, Factory header 10m trend, WebSocket LATEST chart append; CloudInfra Datastores 용량/여유량 카드와 최근 1시간 상태 흐름 시간축 보정 로컬 구현), `apps/cloud-infra-collector/`(Fast/Slow collector, ADR 0027; RDS allocated/max storage, Redis memory usage percent 수집 필드 확장), `infra/data-dashboard/`(ECS Auto Scaling·task role Cognito 권한·metadata env), 관련 ADR 0025/0026/0028/0029/0030/0031, `docs/ops/22_data_dashboard_vpc_runbook.md`.
- 검증: backend `pytest -q` 100 passed, `tests/test_history.py` 35 passed, web lint/test/build 통과(`npm test -- --run` 59 passed), Terraform fmt/validate 기존 통과 및 post-apply plan No changes, GitHub Actions dashboard-backend/web 성공, ECS backend revision 38 `sha-9c28603` rollout completed, desired/running 2, task 2개 HEALTHY(AZ 1a/1c), `/healthz` ok, `/readyz` dynamodb/redis/rds_metadata ok, dashboard web HTTP 200. 2026-06-05 로컬 추가 검증: `apps/cloud-infra-collector` `pytest -q` 16 passed.
