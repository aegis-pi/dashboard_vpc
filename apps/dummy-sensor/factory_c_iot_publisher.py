#!/usr/bin/env python3
"""Publish factory-c dummy outbox JSON files to AWS IoT Core."""
...
class FactoryCIotPublisher:
    def __init__(self, mqtt_client: Any | None = None) -> None:
        ...
        self.data_plane_instance_id = os.getenv(
            ...
            f"factory-c-dummy-publisher-{socket.gethostname()}",
        )
    ...
    def _build_mqtt_client(self) -> MqttClient:
        iot_dir = Path(os.getenv("AEGIS_IOT_DIR", "/etc/aegis/iot/factory-c"))
        ...
        return MqttClient(
            ...
            client_id=os.getenv("AEGIS_IOT_CLIENT_ID", os.getenv("AEGIS_IOT_THING_NAME", "AEGIS-IoTThing-factory-c")),
            ca_file=ca_file,
            cert_file=cert_file,
            key_file=key_file,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Publish factory-c dummy outbox JSON files to AWS IoT Core.")
    ...
    publisher = FactoryCIotPublisher()
