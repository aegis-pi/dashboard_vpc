...
function TimelineTab({ factoryId, refreshSignalKey }: { factoryId: string; refreshSignalKey: number }) {
  ...
  const handleStartChange = useCallback((val: string) => {
    ...
    if (next.end) setCustomEnd(next.end)
  }, [customEnd, nowTick])

  const handleEndChange = useCallback((val: string) => {
    const next = clampTimelineEndValue(val, customStart, nowTick)
    if (!next) return
    setCustomEnd(next.end)
    if (next.start) setCustomStart(next.start)
  }, [customStart, nowTick])
  ...
  const visibleHistory = mode === 'custom' && customRange.valid
    ...
    : history
  const boundaryContext = mode === 'custom' && customRange.valid
    ? history.slice().reverse().find((item) => {
        const ts = item.timestamp ? new Date(item.timestamp).getTime() : NaN
        return Number.isFinite(ts) && ts < customRange.startMs
      })
    : undefined
  const events = deriveTimelineEvents(visibleHistory, boundaryContext)
    ...
    <div className="card">
      <div className="card-hd">
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 10 }}>
          ...
          <span className="micro">{sourceLabel} · {visibleHistory.length} pts · {events.length}건</span>
        </div>
        <div className="seg">
      ...
      {mode === 'custom' && !customRange.valid ? (
        ...
        <EmptyNote text={
          visibleHistory.length === 0
            ...
            : `데이터 ${visibleHistory.length}건은 조회됐지만 risk level 변화나 score 10점 이상 변화가 없습니다.`
        } />
      ) : (
