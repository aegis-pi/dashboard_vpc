...
function App() {
  ...
  const [lastPushAt, setLastPushAt] = React.useState(window.NOW - 3_000);
  ...
  const factory = route.name === "factory"
    ? window.FACTORIES.find(f => f.factory_id === route.id)
    : null;
  const hasAccess = route.name !== "factory"
    || (factory && window.canAccessFactory(t.role, factory.factory_id));
