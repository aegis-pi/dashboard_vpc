...
| 현재 단계 (본 환경 = 워크스트림 B) | **Phase 1 Step 0~3 진입 준비** — 1번 Data / Dashboard VPC 통합 구현 |
...
| 현재 완료 범위 | M0 `factory-a`, M1 Issue 0~10/12, M2 Issue 1~6, M3 Issue 1/4 완료. EKS Hub Tailscale Operator, `factory-a-master` K3s API TCP reachability, ArgoCD/Grafana Tailscale IP UI 접근, ArgoCD `factory-a` cluster 등록, `factory-a-podinfo-smoke` Sync/Healthy, Tailscale egress 장애/복구 검증 완료. IoT Rule -> S3 raw 적재, IRSA S3/AMP 권한, Hub Prometheus Agent -> AMP remote_write, 내부 Grafana -> AMP query, AWS Load Balancer Controller, Admin UI HTTPS Ingress, `factory-a-master` Tailnet 참여와 Windows 운영자 PC SSH 검증, `aegis-pi-gitops` GitOps 저장소 구조와 manifest validation 완료. `aegis-spoke` ApplicationSet 활성 |
| 현재 AWS 상태 | 2026-05-15 rebuild 완료. Hub EKS / Foundation S3·AMP·ECR / IoT Thing·Policy·Rule / K3s IoT Secret / Admin UI ACM·ALB·HTTPS 활성. 1번 Data / Dashboard VPC 는 아직 미배포 |
| 다음 작업 (본 환경) | Phase 1 Step 0 (Gabia 도메인) ↔ Step 1 (Frontend Vite + React 마이그레이션) 병행 → Step 2 `infra/data-dashboard/` Terraform 골격 → Step 3 데이터 저장소. 상세는 `docs/AI_AGENT_HARNESS.md` § 5.4 와 `docs/planning/16_data_dashboard_vpc_workplan.md` |
| 비용 기준 | Hub 상시 가동 시 Admin UI ALB 포함 고정 비용 ~$0.36/hour. Phase 1 데모 운영(월 2회 × 8h) ~$8~10/월, 상시 가동 ~$125/월, destroy 후 ~$2~3/월. 상세 기준은 `docs/ops/15_aws_cost_baseline.md` |
...
| 본 환경 워크스트림 B Phase 1 | ECS Fargate Backend + RDS PostgreSQL + Redis + WebSocket + Bedrock 일간 보고서 통합 배포 | **현재 진입 단계 (Step 0~3)** — `docs/AI_AGENT_HARNESS.md` § 5.4 |
